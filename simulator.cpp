...
void simulator::run(){
   ...
   // value_of_registers[2]=10;
    ...
    if(text_found==-1){
     ...
     for(int j=data_found+1;j<text_found;j++){ //extraaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
        int x=-1;
        string s = program[j];
        //cout<<s<<"[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[["<<endl;
        RemoveSpaces(s);
        cout<<s<<"]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]"<<endl;
         x = s.find(":");
         cout<<x<<"lllllllllllllllllllllllllllllllllllllllllll"<<endl;
         string list_name;
         memory_elements m;
        // RemoveSpaces(list_name);
         if(x!=-1 && x!=0){
            list_name=s.substr(0,x);
            cout<<list_name<<"qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq"<<endl;
            m.name=list_name;
...
simulator::simulator(ifstream& file){
   /* for(int i=0;i<32;i++){
    ...
    cout<<number_of_instructions<<"55555555555555555555555555555555555555"<<endl;
    file.close();  
}
void simulator::read_instruction(int line_number){
    ...
    /////////////////////////////////should writefor labels;
    cout<<present_instruction<<"0000000000000000000000000000"<<endl;
   // RemoveSpaces(present_instruction);
    int count=0;
    ...
    if(is_label==-1){
    cout<<position_in_array<<"2222222222222222222222222222222222"<<endl;
    if(position_in_array==-1){
        ...
        cout<<"yoooooooooooooooooooooo"<<"           "<<registers_in_instruction<<endl;
        registers_in_present_instruction[3]={0};
...
void simulator::valid_register(string s,int n){
    ...
    if(n==6 && x==3){
        ...
        for(int i=0;i<2;i++){
            for(int j=0;j<32;j++){
                if(tokens[i]==registers[j]){
                    cout<<"mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"<<tokens[i]<<"mmmmm"<<registers[j]<<endl;
                    registers_in_present_instruction[i]=j;
        ...
        y++;
        cout<<y<<"yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy"<<endl;
        if(y!=3){
            cout<<"all registers given are not valid";
        }
    }
